...
def ecat_digitizer( ):
    ...
    ddt = DeadTime(volume = sys.block,t = 3000.0,mode='paralysable')
...
def cylindricalPET_digitizer(sys:CylindricalPET):
    ad = Adder()
    rdr = Readout(depth=1)
    blur = Blurring(resolution= 0.26, eor = 511.0)
    thres = ThresHolder(value= 250.0)
...
def optical_digitizer(sys:OpticalSystem):
    ad_op = AdderOptical()
    rdr = Readout(depth=1)
...
def spect_digitizer(sys:SPECThead):
    pass
